document.addEventListener('DOMContentLoaded', (event) => {
  ...
  function handleCardClick(event) {
    ...
    // If the cards are not clickable, return early
    if (!clickable) return;

    // If the clicked card is the same as the last clicked card, return early
    if (event.target === lastClickedCard) return;

    // Set the background color of the clicked card to its class
    ...
    // If this is the second card that was flipped
    if (flippedCards === 2) {
      ...
      if (lastClickedCard.className === colorClass) {
        ...
        // Check if all pairs of colors have been matched
        let allMatched = document.querySelectorAll('.matched').length === COLORS.length * 2;
        ...
        // If the cards do not match, flip them back over after a delay
